...
Parser::terminal_symbol_t  Parser::lexer( char c_ ) const
{
    switch( c_ )
    {
        ...
        case '*':  return terminal_symbol_t::TS_PRODUTO;
        case '/':  return terminal_symbol_t::TS_DIVISAO;
        case '^':  return terminal_symbol_t::TS_POTENCIA;
        case '(':  return terminal_symbol_t::TS_SCOPE1;
        case ')':  return terminal_symbol_t::TS_SCOPE2;
        case '1':
        case '2':
        case '3':
        case '4':
...
bool Parser::expression()
{
    ...
    while( m_result.type == ResultType::OK )
    {
        ...
        else if( accept( Parser::terminal_symbol_t::TS_PRODUTO ) )
        {
            // Stores the "*" token in the list.
            m_tk_list.emplace_back( Token( "*", Token::token_t::OPERATOR ) );
        }
        else if( accept( Parser::terminal_symbol_t::TS_DIVISAO ) )
        {
            m_tk_list.emplace_back( Token( "/", Token::token_t::OPERATOR ) );
        }
        else if( accept( Parser::terminal_symbol_t::TS_POTENCIA ) )
        {
            m_tk_list.emplace_back( Token( "^", Token::token_t::OPERATOR ) );
        }
        else if( accept( Parser::terminal_symbol_t::TS_SCOPE1 ) )
        {
            m_tk_list.emplace_back( Token( "(", Token::token_t::SCOPE ) );
        }
        else if( accept( Parser::terminal_symbol_t::TS_SCOPE2 ) )
        {
            m_tk_list.emplace_back( Token( ")", Token::token_t::SCOPE ) );
        }
        else break;
